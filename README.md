# Sales_data_analysis
This project focuses on analyzing sales data to understand how products, regions, and time periods contribute to overall business performance. The goal was to turn raw sales records into clear insights that can help management make better decisions.

# Problem statement
In this project, I aimed to answer questions such as:
Which products generate the most revenue?
How do sales trends change over time?
Which regions perform best and which need improvement?
 
# Tools used
Power Query (Data Cleaning and Transformation)
Power BI (Data visualization and dashboard creation)

# What I Did
I Cleaned the dataset by handling missing values and correcting inconsistencies.
Organized the data to make it suitable for analysis.
Analyzed sales performance by product, region, and time period.
Created interactive dashboards in Power BI to visualize trends and key performance indicators.

# Key Insights
A small group of products contributed a large percentage of total revenue.
Sales showed noticeable seasonal patterns across different months.
Some regions consistently underperformed compared to others, indicating areas for possible improvement.

# Dashboard
The Power BI dashboard allows users to:
Track total sales and revenue trends
Compare performance across regions
Identify top-performing and low-performing products

# Conclusion
This project shows how sales data can be transformed into meaningful insights.The analysis highlights opportunities for better inventory planning, regional strategy adjustments, and improved decision-making.
 





